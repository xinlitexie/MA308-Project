...
ggplot(tag_analysis_avoid_extreme_top, aes(x = reorder(topics, avg_play), y = avg_play)) +
  ...
  coord_flip() + # 翻转坐标轴便于阅读标签
  labs(
    title = "Top 10 Tags by Average Play Count(Freq > 10)",
    x = "Tags",
    y = "Average Play Count"
  ) +
  theme_minimal() +
...
ggplot(tag_analysis_avoid_extreme_bottom, aes(x = reorder(topics, avg_play), y = avg_play)) +
  ...
  coord_flip() + # 翻转坐标轴便于阅读标签
  labs(
    title = "Bottom 10 Tags by Average Play Count(Freq > 10)",
    x = "Tags",
    y = "Average Play Count"
  ) +
  theme_minimal() +
...
ggplot(model_all_tags, aes(x = estimate, y = reorder(tag_name, estimate), color = ifelse(estimate > 0, "Positive", "Negative"), alpha = significance)) +
  ...
  labs(title = "All Tags Impact on Play Count (Tags are selected from tags above)",
...
leverage_mask[leverage_mask[] == TRUE]
...
cat("解释方差比例: ", round(explained_variance1 * 100, 2), "%\n")
explained_variance2 <- result2$kmeans_result$betweenss / result2$kmeans_result$totss
cat("解释方差比例: ", round(explained_variance2 * 100, 2), "%\n")
...
# 构建 4 级身份因子，并指定 Level 顺序（这对 coin 的比较方向很重要）
data_id <- data_id %>%
  mutate(
...
run_coin_test <- function(data, group_col, value_col, pair, n_perm = 10000) {
  ...
  boot_diffs <- replicate(2000, {
    mean(sample(group2_vec, replace = TRUE)) - mean(sample(group1_vec, replace = TRUE))
  })
  ...
  list(
    ...
    statistic = statistic(test_result)
  )
}
...
p1 <- ggplot(results_final, aes(x = comparison, y = mean_diff, fill = sig)) +
  geom_col(alpha = 0.8) +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.2) +
  geom_text(aes(label = sig), color = "red", size = 5, vjust = -1) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  scale_fill_manual(values = c("***" = "#E74C3C", "**" = "#E74C3C", "*" = "#E67E22", "ns" = "gray80")) +
  labs(
    title = "身份价值差异：置换检验", 
    subtitle = "Mean Difference with 95% Bootstrap CI",
    x = "比较组 (后者 vs 前者)",
    y = "平均播放量差异",
    fill = "Bonferroni\n显著性"
  ) +
...
identity_stats <- data.frame(
  ...
  Avg_Play = c(
    ...
    mean(data_id$play_count[data_id$identity_4level == "None"], na.rm=TRUE)
  )
)

p2 <- ggplot(identity_stats, aes(x = reorder(Identity, -Avg_Play), y = Avg_Play, fill = Identity)) +
  geom_col(width = 0.6, alpha = 0.9) +
  geom_text(aes(label = comma(Avg_Play, accuracy = 1)), vjust = -0.5, size = 4) +
  scale_y_continuous(labels = comma, expand = expansion(mult = c(0, 0.15))) +
  scale_fill_manual(values = c("gray70", "#3498DB", "#E74C3C", "#F1C40F")) + # 调整配色区分度
  labs(
    title = "作者身份价值评估",
    x = "身份类型",
    y = "平均播放量"
...
top_week_data <- data_time_week %>%
  slice_max(order_by = avg_play, n = 1) 

top_week_name <- top_week_data$day_of_week
top_week_val  <- top_week_data$avg_play
...
ggplot(data_time_week, aes(x = day_of_week, y = avg_play, fill = day_of_week)) +
  ...
  labs(
    title = "不同发布时间的流量差异",
    subtitle = paste0("数据洞察：", top_week_name, " 发布的歌单平均播放量最高 (", comma(top_week_val), ")"),
    x = "发布时间",
    y = "平均播放量"
  ) +
  theme_minimal() +
...
# ====模块11：热词检测 (Text Analysis - Impact on Play Count)====
...
# ====模块12：神经网络预测以及error矩阵 (Neural Network Prediction)====
...
# 输入层 = PCA特征 (PC1-PC4) + 结果数据 (收藏/分享/评论)
...
# size = 隐层节点数, decay = 权重衰减(防止过拟合), linout = TRUE 
